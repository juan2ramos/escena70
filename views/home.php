<section class="<?php echo($url) ?>">

    <div id="slide">
        <figure>
            <img src="images/banner1.jpg" alt="banner 1">
        </figure>
    </div>
    <article class="first">
        <p>
            Para conmemorar los <b>70 años de Teatro Municipal</b>, el Complejo Teatral de
            Buenos Aires realizará por única vez<b> Escena 70 – Mercado de Artes Escénicas de Buenos
                Aires.</b>
        </p>
        <p>
            <b>Escena 70 - Mercado de Artes Escénicas de Buenos Aires</b> desea propiciar la creación de una plataforma exportadora para las artes escénicas de Buenos Aires que permita proyectar internacionalmente la vasta producción artística de esta parte del mundo.
        </p>
        <p>
            Durante tres días empresarios culturales, productores, managers, creadores, especialistas en marketing cultural y artistas, encontrarán en <b>Escena 70</b> la oportunidad ideal para conocer las mejores propuestas de teatro, danza, performance y títeres producidas en Buenos Aires, intercambiar experiencias de negocios, ampliar los circuitos de recepción y consumo de sus productos culturales, negociar las condiciones de presentación y circulación, y sobre todo, abrir las puertas del mundo a la diversidad cultural porteña.
        </p>

        <p>Organiza Complejo Teatral de Buenos Aires</p>

        <p>En colaboración con Redlat Colombia / Circulart</p>

        <p>Con la participación de: Grupo La Plaza, Teatro Picadero y el Cultural San Martín</p>


    </article>
    <article class="second">
        <h3>Información sobre la convocatoria</h3>

        <p><b>13 de mayo:</b> Apertura inscripciones en la plataforma
            <a href="http://escena70.complejoteatral.gob.ar">http://escena70.complejoteatral.gob.ar</a>
        </p>

        <p><b>20 de julio: </b>Cierre inscripciones </p>

        <p><b>Primera semana de agosto:</b> Publicación de resultados
            <b>6 de septiembre:</b> Apertura agendamiento de citas entre Artistas y Profesionales </p>

        <p><b>27 de septiembre:</b> Cierre agendamiento </p>

        <p><b>6 al 8 de octubre:</b> Escena 70 - Mercado de Artes Escénicas de Buenos Aires - Muestras artísticas y
            Ruedas de negocios. </p>

        <p>
            Cualquier información adicional puede comunicarse al correo
            <a href="mailto:convocatoriaescena70@complejoteatral.gob.ar">convocatoriaescena70@complejoteatral.gob.ar</a>
        </p>


    </article>

</section>
<div class="divisor">
    <div class="lines"></div>
    <span class="plus">+</span>
</div>
<section class="<?php echo($url) ?>">
    <article class="first image">
        <figure>
            <img src="images/foto1.jpg" alt="foto 1"/>
            <figcaption>
                <p>Teatro Municipal <span>&nbsp&nbspGeneral San Martín</span>
                </p>
            </figcaption>
        </figure>
    </article>
    <article class="second article-contend">
        <div class="title-conted">
            <hr class="color-hr"/>
            <h2>mercado de artes escénicas de</h2></div>
        <div class="title-conted">
            <hr/>
            <h2><b>buenos aires</b></h2>
        </div>
        <div class="article-last">
            <h3>6 AL 8 DE OCTUBRE</h3>

            <h3>2014</h3>

            <p>Escena 70 - Mercado de Artes Escénicas de Buenos Aires</p>
            <h4>_Muestras artísticas</h4>
            <h4>_Ruedas de negocios</h4>
        </div>

    </article>
</section>
<div class="divisor">
    <div class="lines"></div>
    <span class="plus">+</span>
</div>